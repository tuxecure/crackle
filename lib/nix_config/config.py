        ...
        with open(filename, 'r') as file:
            ...
            for i, line in enumerate(lines): # i is one behind where we actually are.
                ...
                if '.packages' in line and not stripped_line.startswith('#'):
                    start_line = i
                    package["start_indent"] : int = len(line) - len(line.lstrip())
                    package_type = line.split('.packages')[0].strip()
                    package["package_type"] : str = package_type # we want to make sure we know where we are installing this package home system?, we want to save that.
                    package["start_line"] : int = i + 1 # we want to save the start line so we know where to start putting things later (the + 1 is to make sure we are on lines where we describe packages)
                elif '.systemPackages' in line and not stripped_line.startswith('#'): # if systemPackages
                    start_line = i
                    package["start_indent"] : int = len(line) - len(line.lstrip())
                    package["package_type"] : str = "system" # we want to make sure we know where we are installing this package home system?, we want to save that.
                    package["start_line"] : int = i + 1 # we want to save the start line so we know where to start putting things later (the + 1 is to make sure we are on lines where we describe packages)
                elif package["start_line"] is not None and not stripped_line.startswith('#') and len(line) - len(line.lstrip()) == package["start_indent"] and '];' in line: # check indent and if correct and not comment then this is the end of .packages
                    package["end_line"] : int = i  # we want to save the end line so we know where to stop putting things later
                    package_blocks.append(copy.deepcopy(package)) # add package to packages
                    package : dict = {
                        "package_type" : None,
                        "start_line" : None,
                        "end_line" : None,
...
def check_package_blocks(package_blocks: list != []) -> bool:
    """
    ...
    for package_type in package_block_types:
        if package_type in seen_types:
            print(f"Error: Duplicate package block of type '{package_type}' this is not supported.")
            return True
...
def read_packages(filename : str, package_type : str) -> list:
    """
        ...
        for package_block in package_blocks:
            if package_block["package_type"] == package_type:
                ...
                with open(filename, 'r') as file:
                    ...
                    lines = lines[package_block["start_line"]:package_block["end_line"]] # make sure to get only the package block and not whole file.
                    for line in lines: # i is one behind where we actually are.
                        stripped_line = line.strip()
                        if not stripped_line.startswith('#') and '(' not in stripped_line and ')' not in stripped_line and '[' not in stripped_line and ']' not in stripped_line and stripped_line != '': # filter comments, newlines and unsupported stuff.
...
def add_packages(filename : str, packages : list, package_type : str, overwrite : bool = False) -> list:
    """
    ...
    processed_packages : list = [] # fix for nix search
    ...
    package_blocks : list = process_file(filename)
    existing_packages : list = read_packages(filename, package_type) if not overwrite else [] # read packages so we can add them back in if override not needed.

    if check_package_blocks(package_blocks): # check package_blocks
        return [] # if we need to exit specify packages added, which is zero.

    for package in existing_packages: # readd existing packages to packages so we won't override them.
    ...
    try:
        for package_block in package_blocks:
            if package_block["package_type"] == package_type:
                with open(filename, 'r') as file:
                    lines = file.readlines()  # read lines from file
                lines[package_block["start_line"]:package_block["end_line"]] = [' ' * (package_block["start_indent"] + 2) + package + '\n' for package in packages]
...
def delete_packages(filename : str, packages : list, package_type : str = None) -> list:
    """
    ...
    deleted_packages : list = [] # total of deleted packages
    package_blocks : list = process_file(filename)
    ...
    if not package_type: # default to checking everywhere for package
        ...
        for package_block in package_blocks: 
            package_block_types.append(package_block["package_type"])
        for package_type in package_block_types:
            if not package_type in seen_types: # we know we don't need this because of check_package_blocks but sanity.
                seen_types.add(package_type)
    ...
    for package_type in seen_types:
        existing_packages : list = read_packages(filename, package_type)
        packages_to_delete: list = [package for package in packages if package in existing_packages] # check what we can delete
        updated_packages: list = [package for package in existing_packages if package not in packages_to_delete] # remove what we need to delete from existing packages

        add_packages(filename, updated_packages, package_type, True) # delete packages

        for package in packages_to_delete: # read packages_to_delete and add that to total packages deleted
